...
**Se o RG do cliente vier com os mesmos dígitos do CPF, não é erro de digitação** (caso Rosely x Pague Menos, 2026-07-28) — desde o Decreto n.º 10.977/2022, a nova Carteira de Identidade Nacional (CIN) unificou o "Registro Geral" ao número do CPF para quem já emitiu o documento no modelo novo. Antes de perguntar ao cliente ou marcar como "a confirmar", verificar se há uma Carteira de Identidade digital (`gov.br`/gov.br Carteira de Identidade, às vezes salva como PDF tipo "wallet_RG-DIGITAL.pdf" numa exportação de conversa) nos documentos do caso — ela confirma visualmente o RG = CPF e traz também o órgão expedidor (geralmente o instituto de identificação do estado, ex.: POLITEC/MT), naturalidade e data de nascimento. Confirmado isso, usar o mesmo número para RG e CPF na qualificação, citando o Decreto 10.977/2022, em vez de tratar como pendência.

**O CNPJ da reclamada, quando não fornecido pelo cliente, normalmente já está no TRCT (campo "01 CNPJ/CEI" do cabeçalho "IDENTIFICAÇÃO DO EMPREGADOR") ou na CTPS Digital (campo "Estabelecimento" do contrato de trabalho ativo)** — conferir esses dois documentos antes de marcar o CNPJ/endereço da reclamada como pendência ou de reaproveitar o CNPJ de um caso anterior contra o mesmo empregador (cada loja/filial tem CNPJ próprio — reaproveitar o CNPJ de outra unidade da mesma rede é um erro, como quase ocorreu no caso Rosely ao encontrar por engano o CNPJ da filial de Várzea Grande do caso Geovane em vez da filial de Cuiabá da própria Rosely).
...
### 5. Integração das verbas para fins de cálculo / primazia da realidade (gatilho: classificar cada verba pedida como remuneratória ou não — havendo qualquer verba remuneratória, ex.: horas extras, adicional de transferência, comissão, gratificação, prêmio, "pagamento por fora", adicional noturno/insalubridade/periculosidade, incluir esta cláusula E a cláusula 6 juntas, uma para cada verba remuneratória do caso)
...
### Matriz de provas por tópico e "resumo prático" (client-facing)
Ver "PARTE 3 — Matriz de provas por tópico" no catálogo consolidado (`catalogo-consolidado-peticao-trabalhista.md`) — classifica cada um dos 38 tópicos por tipo de prova necessária (PT/PD/DP/PER), usada tanto para planejar a instrução (rol de testemunhas, quesitos periciais) quanto para o **resumo prático** que vai para o cliente (formatado para copiar/colar no WhatsApp, negrito de asterisco simples, sem valores/sem ressalvas — ver [[project-caso-rosely-moraes]] para o modelo real usado).

**Regra para todo item marcado PER no resumo prático**: incluir uma linha própria de pedido, logo abaixo do nome do tópico, no padrão — "Requer a realização de perícia [técnica/médica] para fins de dirimir o exercício de atividade insalubre/periculosa/doença ocupacional (com base no contexto da ação)" — ajustando modalidade e fundamento ao caso, sem nenhum outro comentário na mesma linha (mantém o resumo objetivo, mas garante que o pedido de prova essencial não se perca entre o dossiê técnico e a petição final).
...
## Regra: nunca colocar comentários internos na petição

A petição em si não deve conter nenhuma linguagem de análise interna do escritório: rótulos de "força da tese" (forte/moderado/fraco), ressalvas do tipo "sem prejuízo de o Juízo concluir de forma diversa", notas preventivas antecipando defesa da reclamada (ex.: sobre CCT) que soem como dúvida própria, ou qualquer menção a "achado"/"estratégia"/nomes de skills internas. A petição sempre afirma a tese como correta — essas análises (força de cada requisito por período, contra-argumentos previstos, etc.) ficam **só** no dossiê/análise interna do caso, nunca no documento que vai ao Judiciário (decisão do escritório, caso Rosely x Pague Menos, 2026-07-29, após o cabeçalho de cada tópico ter vazado rótulos como "(Força Forte)"/"(Força Moderada)" para a petição por engano).
...
- **Negrito**: reservado para (a) as linhas "TOTAL:"/"TOTAL DO PEDIDO:", (b) a declaração "Dá-se à causa o valor de R$ [...]", (c) nome do advogado + OAB na assinatura final, e (d) o nome completo do(a) reclamante e da reclamada na primeira menção, dentro do endereçamento/qualificação (confirmado na Tayná: nome próprio em negrito, resto da qualificação em texto normal). Nunca negritar a prosa de fundamentação nem os itens individuais de bullet (verba/reflexo) — só o total consolidado.
- **Cabeçalho/endereçamento em CAIXA ALTA e negrito**: o "AO JUÍZO DA... VARA DO TRABALHO DE..." e o nome do rito ("AÇÃO TRABALHISTA PELO RITO ORDINÁRIO") usam o estilo `Heading 1` (que já aplica negrito e centralização) — mas **sem cor**: o tema padrão do `python-docx` (usado pelo `md_to_docx.py`) aplica azul às cores de heading por padrão, o que não existe na Tayná real (título preto/automático, só negrito). Sempre forçar a cor preta explícita nos estilos de Heading ao gerar o `.docx` (ver `md_to_docx.py`, corrigido em 2026-07-29) — do contrário, os títulos saem em azul, fora do padrão do escritório.
...
2. Inserir as cláusulas universais sempre: 0 (cabeçalho/qualificação/nome da ação), 1 (gratuidade), 2 (Juízo 100% Digital), 4 (valores estimativos/não limitação), 8 (multa do art. 467 — controvérsia sobre o montante quase sempre presente na prática), 10 (honorários, 15%), 11 (intimação exclusiva), 12 (provas).
2.1. **Classificar cada verba pedida como remuneratória ou não** (ver lista na cláusula 5). Havendo qualquer verba remuneratória (horas extras, adicional de transferência, comissão, gratificação, prêmio, adicional noturno/insalubridade/periculosidade etc.), incluir a cláusula 5 (integração/primazia da realidade) e a cláusula 6 (cadeia de reflexos) juntas para cada uma delas — normalmente se aplica na maioria dos casos, mas não pular a checagem.
