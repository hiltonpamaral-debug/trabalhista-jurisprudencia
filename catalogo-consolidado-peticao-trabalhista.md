...
**Como calcular a remuneração real (ferramenta):** usar a skill `conciliar-remuneracao-real-trabalhista` (compara CNIS/FGTS mês a mês contra o salário-base declarado) e, se houver TRCT, a skill `auditar-trct-trabalhista` (confere se as rubricas de 13º/férias/aviso-prévio usaram a média real correta).

**Estratégia de prova quando não há contracheques (caso Rosely x Pague Menos, 2026-07-28):** o CNIS é documento de terceiro (Dataprev/INSS), com presunção de veracidade — pode servir de base de cálculo mesmo sem contracheques. A guarda de contracheques por 5 anos é obrigação legal do empregador; requerer na petição a exibição pela reclamada (art. 396 CPC) e, na ausência, a presunção de veracidade dos cálculos apresentados, por aplicação analógica da Súmula 338 TST (presunção em favor do empregado quando o empregador, tendo o dever de guardar o documento, não o apresenta).
...
2. Teste de primazia da realidade (cross-check obrigatório, metodologia validada no caso Rosely x Pague Menos, 2026-07-28): usando a remuneração real mês a mês (CNIS oficial, preferencialmente — ver skill `conciliar-remuneracao-real-trabalhista`), calcular a média FIXA dos 12 meses anteriores a cada mês da série, e verificar se a remuneração se mantém ≥1,4x essa média por pelo menos 6 meses seguidos — **sem recalcular a média com os meses já elevados** (senão o teste nunca dispara, porque a própria média sobe junto). Rodar esse teste na série toda (não só no mês da promoção) identifica os "marcos" reais de aumento de 40% — se a data da promoção formal não coincidir com nenhum marco real, o requisito objetivo falha, mesmo que a gratificação nominal bata 40% do salário-base num holerite isolado. A empresa pode reclassificar rubricas na promoção pra forçar o teste formal bater sem o ganho total ter mudado — se os dois testes divergirem, é indício de fraude na composição das rubricas.
